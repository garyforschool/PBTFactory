...
def _process_traceback(exc: BaseException, te: (traceback.
    TracebackException | None)=None, *, skip: int=0, hide: bool=True
    ) ->traceback.TracebackException:
    if te is None:
        te = traceback.TracebackException.from_exception(exc, lookup_lines=
            False)
    ...
    for (f, _), fs in zip(frame_gen, te.stack):
        if hide:
            ...
            if hide_value in {'before', 'before_and_this'}:
                ...
                continue
        frame_args: dict[str, t.Any] = {'filename': fs.filename, 'lineno':
            fs.lineno, 'name': fs.name, 'locals': f.f_locals, 'globals': f.
            f_globals}
        if hasattr(fs, 'colno'):
    ...
    if te.__context__:
        ...
        te.__context__ = _process_traceback(context_exc, te.__context__,
            hide=hide)
    if te.__cause__:
...
class DebugTraceback:
    ...
    def __init__(self, exc: BaseException, te: (traceback.
        TracebackException | None)=None, *, skip: int=0, hide: bool=True
        ) ->None:
    ...
    @cached_property
    def all_tracebacks(self) ->list[tuple[str | None, traceback.
        TracebackException]]:
        out = []
        current = self._te
    ...
    def render_traceback_html(self, include_title: bool=True) ->str:
        ...
        return SUMMARY_HTML % {'classes': classes, 'title':
            f'<h3>{title}</h3>', 'frames': '\n'.join(rows), 'description':
            description}

    def render_debugger_html(self, evalex: bool, secret: str,
        evalex_trusted: bool) ->str:
        ...
        if sys.version_info < (3, 13):
            ...
            exc_type_str = self._te.exc_type_str
        return PAGE_HTML % {'evalex': 'true' if evalex else 'false',
            'evalex_trusted': 'true' if evalex_trusted else 'false',
            'console': 'false', 'title': escape(exc_lines[0]), 'exception':
            escape(''.join(exc_lines)), 'exception_type': escape(
            exc_type_str), 'summary': self.render_traceback_html(
            include_title=False), 'plaintext': escape(plaintext),
            'plaintext_cs': re.sub('-{2,}', '-', plaintext), 'secret': secret}


class DebugFrameSummary(traceback.FrameSummary):
    __slots__ = ('local_ns', 'global_ns', '_cache_info',
        '_cache_is_library', '_cache_console')

    def __init__(self, *, locals: dict[str, t.Any], globals: dict[str, t.
        Any], **kwargs: t.Any) ->None:
    ...
    def is_library(self) ->bool:
        return any(self.filename.startswith((path, os.path.realpath(path))) for
            path in sysconfig.get_paths().values())
...
<span class="ws">{' ' * prefix}</span>{' ' * (colno - prefix)}{'^' * (end_colno - colno)}"""
        ...
        return FRAME_HTML % {'id': id(self), 'filename': escape(self.
            filename), 'lineno': self.lineno, 'function_name': escape(self.
            name), 'lines': '\n'.join(rendered_lines), 'library': 'library' if
            mark_library and self.is_library else ''}


def render_console_html(secret: str, evalex_trusted: bool) ->str:
    return CONSOLE_HTML % {'evalex': 'true', 'evalex_trusted': 'true' if
        evalex_trusted else 'false', 'console': 'true', 'title': 'Console',
        'secret': secret}
