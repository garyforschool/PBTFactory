...
def test_app(req: Request) ->Response:
    try:
        ...
        eggs = sorted(pkg_resources.working_set, key=lambda x: x.
            project_name.lower())
    python_eggs = []
    for egg in eggs:
        try:
    ...
    wsgi_env = []
    sorted_environ = sorted(req.environ.items(), key=lambda x: repr(x[0]).
        lower())
    ...
    context = {'python_version': '<br>'.join(escape(sys.version).splitlines
        ()), 'platform': escape(sys.platform), 'os': escape(os.name),
        'api_version': sys.api_version, 'byteorder': sys.byteorder,
        'werkzeug_version': _get_werkzeug_version(), 'python_eggs': '\n'.
