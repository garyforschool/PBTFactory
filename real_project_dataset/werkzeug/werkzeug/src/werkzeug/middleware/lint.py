...
class InputStream:
    ...
    def read(self, *args: t.Any) -> bytes:
        if len(args) == 0:
            warn(
                "WSGI does not guarantee an EOF marker on the input stream, thus making calls to 'wsgi.input.read()' unsafe. Conforming servers may never return from this call.",
                WSGIWarning,
                stacklevel=2,
            )
        elif len(args) != 1:
    ...
    def readline(self, *args: t.Any) -> bytes:
        if len(args) == 0:
            ...
            warn(
                "'wsgi.input.readline()' was called with a size hint. WSGI does not support this, although it's available on all major servers.",
                WSGIWarning,
                stacklevel=2,
            )
        else:
    ...
    def close(self) -> None:
        ...
        if self.headers_set:
            ...
            if status_code == 304:
                ...
                if bytes_sent:
                    warn(
                        "304 responses must not have a body.", HTTPWarning, stacklevel=2
                    )
...
class LintMiddleware:
    ...
    def check_start_response(
        ...
        if len(status) < 4 or status[3] != " ":
            warn(
                f"Invalid value for status {status!r}. Valid status strings are three digits, a space and a status explanation.",
                WSGIWarning,
                stacklevel=3,
            )
    ...
    def check_iterator(self, app_iter: t.Iterable[bytes]) -> None:
        if isinstance(app_iter, str):
            warn(
                "The application returned a string. The response will send one character at a time to the client, which will kill performance. Return a list or iterable instead.",
                WSGIWarning,
                stacklevel=3,
            )
