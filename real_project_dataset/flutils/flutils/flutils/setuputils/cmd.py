...
import shutil
import sys
from typing import ClassVar, Generator, List, Tuple, Type, Union
    ...
    if out:
        if os.access(out, os.X_OK) is False:
            raise PermissionError(
                "You do not have execute permission to run the file: %r" % out
            )
        return out
    raise FileNotFoundError("Unable to find the file path for the command: %r" % cmd)
