...
def _each_setup_cfg_command(
    parser: ConfigParser, format_kwargs: Dict[str, str]
...
def _get_name(parser: ConfigParser, setup_cfg_path: str) -> str:
    try:
        ...
        raise LookupError(
            "The config file, %r, is missing the 'metadata' section." % setup_cfg_path
        )
...
def _validate_setup_dir(setup_dir: str) -> None:
    if os.path.exists(setup_dir) is False:
        raise FileNotFoundError(
            "The given 'setup_dir' of %r does NOT exist." % setup_dir
        )
    if os.path.isdir(setup_dir) is False:
        raise NotADirectoryError(
            "The given 'setup_dir' of %r is NOT a directory." % setup_dir
        )
    path = os.path.join(setup_dir, "setup.py")
    if os.path.isfile(path) is False:
        raise FileNotFoundError(
            "The given 'setup_dir' of %r does NOT contain a setup.py file." % setup_dir
        )
    path = os.path.join(setup_dir, "setup.cfg")
    if os.path.isfile(path) is False:
        raise FileNotFoundError(
            "The given 'setup_dir' of %r does NOT contain a setup.cfg file." % setup_dir
        )
