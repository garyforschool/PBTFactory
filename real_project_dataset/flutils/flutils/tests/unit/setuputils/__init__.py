...
from flutils.setuputils.cmd import _finalize_options, _initialize_options, _run


def _build_class(setup_command_cfg: SetupCfgCommandConfig) -> Type[Command]:
    setup_klass = type(
        "SetupCfgCommand",
        (object,),
        {
...
class TestSetupUtils(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        patcher = patch(
            "flutils.setuputils.each_sub_command_config", return_value=self.args
        )
    ...
    def test_add_setup_cfg_commands(self) -> None:
        ...
        for x, cfg in enumerate(self.args):
            ...
            self.assertEqual(
                setup_kwargs["cmdclass"][cfg.name], self.command_classes[x]
            )
