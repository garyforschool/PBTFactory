...
""",
    ...
    def test_show_command() -> None:
        command = "/a/path/to/test", "-a", "--abc=true", "--def", "false"
    ...
    def test_run__0(self) -> None:
        ...
        patcher = patch("flutils.setuputils.cmd._each_command", return_value=cmds)
        each_command = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.setuputils.cmd._show_command")
        show_command = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.setuputils.cmd.run", side_effect=[0, 0])
        run = patcher.start()
        self.addCleanup(patcher.stop)
        _run(_self)
        each_command.assert_called_once_with(cmds)
        show_command.assert_has_calls([call(cmds[0]), call(cmds[1])])
        run.assert_has_calls([call(cmds[0]), call(cmds[1])])
    ...
    def test_run__1(self) -> None:
        ...
        show_command = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.setuputils.cmd.run", side_effect=[0, 5])
        run = patcher.start()
        self.addCleanup(patcher.stop)
        _run(_self)
        each_command.assert_called_once_with(cmds)
    ...
    def test_build_sub_command_class() -> None:
        ...
        with patch("flutils.setuputils.cmd._type") as type_:
            ...
            type_.assert_has_calls(
                [call("SetupCfgCommand", ANY, ANY), call("MultiCommand", ANY, ANY)]
            )
