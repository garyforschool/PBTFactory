...
class TestCmdutilsRun(unittest.TestCase):
    ...
    def setUp(self) -> None:
        patcher = patch("flutils.cmdutils.shlex.split", return_value=["ls", "-Flap"])
        self.shlex_split = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.cmdutils.shutil.which", return_value="/bin/bash")
        self.shutil_which = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.cmdutils.sys.stdout", spec=sys.stdout)
        self.sys_stdout_buffer = patcher.start()
        type(self.sys_stdout_buffer).encoding = PropertyMock(return_value="utf-8")
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.cmdutils.sys.stderr", new_callable=BytesIO)
        self.sys_stderr_buffer = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.cmdutils.shutil.get_terminal_size", return_value=(115, 25)
        )
        self.shutil_get_terminal_size = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.cmdutils.pty.openpty", side_effect=[(10, 20), (30, 40)]
        )
        self.pty_openpty = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.cmdutils._set_size", side_effect=[None, None, None, None]
        )
        ...
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.cmdutils.select", side_effect=[[(10, 30), None], [(10, 30), None]]
        )
        self.select = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.cmdutils.os.read", side_effect=[b"foo\n", b"bar\n", "", ""]
        )
