...
class TestGetOsUser(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher = patch("flutils.pathutils.getpass.getuser", return_value="test_user")
...
class TestGetOsUserException(unittest.TestCase):
    ...
    def setUp(self):
        patcher = patch("flutils.pathutils.pwd.getpwuid", side_effect=KeyError("Boom!"))
        self.getpwuid = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.getpass.getuser", return_value="bad_user")
        self.getuser = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.pwd.getpwnam", side_effect=KeyError("Boom!"))
        self.getpwnam = patcher.start()
        self.addCleanup(patcher.stop)
