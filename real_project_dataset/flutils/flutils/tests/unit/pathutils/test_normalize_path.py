...
from ..mocks.pathlib import Path, PosixPathMock
...
class TestNormalizePath(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.os.path.isabs", return_value=True)
        self.isabs = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.pathutils.os.getcwd", return_value="/home/test_user/tmp"
        )
        self.getcwd = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.os.path.join", return_value=None)
...
class TestNormalizePathCwd(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.pathutils.os.path.expanduser", return_value="foo/../$TEST"
        )
        self.expanduser = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.pathutils.os.path.expandvars", return_value="foo/../test"
        )
        self.expandvars = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.os.path.isabs", return_value=False)
        self.isabs = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.pathutils.os.getcwd", return_value="/home/test_user/tmp"
