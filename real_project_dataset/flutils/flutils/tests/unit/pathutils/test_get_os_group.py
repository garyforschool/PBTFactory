...
class TestGetOsGroup(unittest.TestCase):
    ...
    def setUp(self):
        patcher = patch(
            "flutils.pathutils.get_os_user", return_value=SimpleNamespace(pw_gid=3287)
        )
...
class TestGetOsGroupException(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.get_os_user = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.grp.getgrgid", side_effect=KeyError("Boom!"))
        self.getgrgid = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.grp.getgrnam", side_effect=KeyError("Boom!"))
        self.getgrnam = patcher.start()
        self.addCleanup(patcher.stop)
