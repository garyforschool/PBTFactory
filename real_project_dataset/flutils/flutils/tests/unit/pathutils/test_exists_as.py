...
class TestExistsAsDirectory(unittest.TestCase):
    ...
    def setUp(self):
        self.path = PosixPathMock("/home/test_user/directory", is_dir=True)
...
class TestExistsAsFile(unittest.TestCase):
    ...
    def setUp(self):
        self.path = PosixPathMock("/home/test_user/file", is_file=True)
        patcher = patch("flutils.pathutils.normalize_path", return_value=self.path)
        self.normalize_path = patcher.start()
        self.addCleanup(patcher.stop)
...
class TestExistsAsBlockDevice(unittest.TestCase):
    ...
    def setUp(self):
        self.path = PosixPathMock("/home/test_user/block_device", is_block_device=True)
        patcher = patch("flutils.pathutils.normalize_path", return_value=self.path)
        self.normalize_path = patcher.start()
        self.addCleanup(patcher.stop)
...
class TestExistsAsCharDevice(unittest.TestCase):
    ...
    def setUp(self):
        self.path = PosixPathMock("/home/test_user/char_device", is_char_device=True)
        patcher = patch("flutils.pathutils.normalize_path", return_value=self.path)
        self.normalize_path = patcher.start()
        self.addCleanup(patcher.stop)
...
class TestExistsAsFifo(unittest.TestCase):
    ...
    def setUp(self):
        self.path = PosixPathMock("/home/test_user/fifo", is_fifo=True)
        patcher = patch("flutils.pathutils.normalize_path", return_value=self.path)
        self.normalize_path = patcher.start()
        self.addCleanup(patcher.stop)
...
class TestExistsAsSocket(unittest.TestCase):
    ...
    def setUp(self):
        self.path = PosixPathMock("/home/test_user/socket", is_socket=True)
        patcher = patch("flutils.pathutils.normalize_path", return_value=self.path)
        self.normalize_path = patcher.start()
        self.addCleanup(patcher.stop)
