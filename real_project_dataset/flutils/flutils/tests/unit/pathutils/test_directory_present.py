...
class TestDirectoryPresent(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = PosixPathMock("/home/test_user/tmp", is_dir=True, exists=True)
        self.dir_one = PosixPathMock(
            "/home/test_user/tmp/dir_one", is_dir=False, exists=False, parent=self.tmp
        )
        ...
        patcher = patch(
            "flutils.pathutils.exists_as", side_effect=["", "", "", "directory"]
        )
        self.exists_as = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.chown", return_value=None)
        self.chown = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.chmod", return_value=None)
...
class TestDirectoryPresentExisting(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = PosixPathMock("/home/test_user/tmp", is_dir=True, exists=True)
        self.dir_one = PosixPathMock(
            "/home/test_user/tmp/dir_one", is_dir=True, exists=True, parent=self.tmp
        )
        self.dir_two = PosixPathMock(
            "/home/test_user/tmp/dir_one/dir_two",
            is_dir=True,
        ...
        patcher = patch(
            ...
            side_effect=["directory", "directory", "directory", "directory"],
        )
        self.exists_as = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.chown", return_value=None)
...
class TestDirectoryPresentGlobError(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher = patch(
            "flutils.pathutils.exists_as",
            side_effect=["", "directory", "directory", "directory"],
        )
        self.exists_as = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.chown", return_value=None)
...
class TestDirectoryPresentAbsoluteError(unittest.TestCase):
    ...
    def setUp(self):
        self.path = PosixPathMock("home/test_user/tmp/dir_one/dir_two", exists=False)
        patcher = patch("flutils.pathutils.normalize_path", return_value=self.path)
        self.normalize_path = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.exists_as", side_effect=["", ""])
        self.exists_as = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.chown", return_value=None)
        self.chown = patcher.start()
...
class TestDirectoryPresentFileError(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = PosixPathMock("/home/test_user/tmp", is_dir=True, exists=True)
        self.path = PosixPathMock(
            "/home/test_user/tmp/path", is_file=True, exists=True, parent=self.tmp
        )
        patcher = patch("flutils.pathutils.normalize_path", return_value=self.path)
        self.normalize_path = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.pathutils.exists_as", side_effect=["file", "directory"]
        )
        self.exists_as = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.chown", return_value=None)
    ...
    def test_directory_present_file_error(self):
        self.assertRaises(FileExistsError, directory_present, self.path.as_posix())
        self.normalize_path.assert_called_with(self.path.as_posix())
        self.path.mkdir.assert_not_called()
        self.tmp.mkdir.assert_not_called()
        self.chmod.assert_not_called()
...
class TestDirectoryPresentBlockDeviceError(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.pathutils.exists_as", side_effect=["block device", "directory"]
        )
        self.exists_as = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.chown", return_value=None)
...
class TestDirectoryPresentCharDeviceError(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.pathutils.exists_as", side_effect=["char device", "directory"]
        )
        self.exists_as = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.chown", return_value=None)
...
class TestDirectoryPresentFifoError(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = PosixPathMock("/home/test_user/tmp", is_dir=True, exists=True)
        self.path = PosixPathMock(
            "/home/test_user/tmp/path", is_fifo=True, exists=False, parent=self.tmp
        )
        patcher = patch("flutils.pathutils.normalize_path", return_value=self.path)
        self.normalize_path = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.pathutils.exists_as", side_effect=["FIFO", "directory"]
        )
        self.exists_as = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.chown", return_value=None)
...
class TestDirectoryPresentSocketError(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = PosixPathMock("/home/test_user/tmp", is_dir=True, exists=True)
        self.path = PosixPathMock(
            "/home/test_user/tmp/path", is_socket=True, exists=False, parent=self.tmp
        )
        patcher = patch("flutils.pathutils.normalize_path", return_value=self.path)
        self.normalize_path = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.pathutils.exists_as", side_effect=["socket", "directory"]
        )
        self.exists_as = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.chown", return_value=None)
...
class TestDirectoryPresentParentFileError(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = PosixPathMock("/home/test_user/tmp", is_file=True, exists=True)
        self.path = PosixPathMock(
            "/home/test_user/tmp/path", exists=False, parent=self.tmp
        )
        patcher = patch("flutils.pathutils.normalize_path", return_value=self.path)
        self.normalize_path = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.exists_as", side_effect=["", "file"])
        self.exists_as = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.chown", return_value=None)
        self.chown = patcher.start()
...
class TestDirectoryPresentParentBlockDeviceError(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = PosixPathMock(
            "/home/test_user/tmp", is_block_device=True, exists=False
        )
        self.path = PosixPathMock(
            "/home/test_user/tmp/path", exists=False, parent=self.tmp
        )
        ...
        self.normalize_path = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.exists_as", side_effect=["", "block device"])
        self.exists_as = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.chown", return_value=None)
        self.chown = patcher.start()
...
class TestDirectoryPresentParentCharDeviceError(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = PosixPathMock(
            "/home/test_user/tmp", is_char_device=True, exists=False
        )
        self.path = PosixPathMock(
            "/home/test_user/tmp/path", exists=False, parent=self.tmp
        )
        ...
        self.normalize_path = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.exists_as", side_effect=["", "char device"])
        self.exists_as = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.chown", return_value=None)
        self.chown = patcher.start()
...
class TestDirectoryPresentParentFifoError(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = PosixPathMock("/home/test_user/tmp", is_fifo=True, exists=False)
        self.path = PosixPathMock(
            "/home/test_user/tmp/path", exists=False, parent=self.tmp
        )
        patcher = patch("flutils.pathutils.normalize_path", return_value=self.path)
        self.normalize_path = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.exists_as", side_effect=["", "FIFO"])
        self.exists_as = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.chown", return_value=None)
        self.chown = patcher.start()
...
class TestDirectoryPresentParentSocketError(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = PosixPathMock("/home/test_user/tmp", is_socket=True, exists=False)
        self.path = PosixPathMock(
            "/home/test_user/tmp/path", exists=False, parent=self.tmp
        )
        patcher = patch("flutils.pathutils.normalize_path", return_value=self.path)
        self.normalize_path = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.exists_as", side_effect=["", "socket"])
        self.exists_as = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.chown", return_value=None)
        self.chown = patcher.start()
