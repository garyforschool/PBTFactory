...
class TestChown(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher = patch(
            "flutils.pathutils.get_os_user", return_value=SimpleNamespace(pw_uid=9753)
        )
        self.get_os_user = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.pathutils.get_os_group", return_value=SimpleNamespace(gr_gid=1357)
        )
        self.get_os_group = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.pathutils.os.chown", return_value=True)
