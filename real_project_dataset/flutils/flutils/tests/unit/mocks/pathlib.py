...
class PosixPathMock:
    ...
    def parent(self):
        if self._parent_ is None:
            self._parent_ = PosixPathMock(
                "/".join(self._path.split("/")[:-1]), is_dir=True, check=True
            )
    ...
    def is_dir(self):
        if self._is_dir_ is None:
            self._is_dir_ = MagicMock(return_value=self.kwargs.get("is_dir", False))
        return self._is_dir_

    @property
    def is_file(self):
        if self._is_file_ is None:
            self._is_file_ = MagicMock(return_value=self.kwargs.get("is_file", False))
    ...
    def is_fifo(self):
        if self._is_fifo_ is None:
            self._is_fifo_ = MagicMock(return_value=self.kwargs.get("is_fifo", False))
    ...
    def is_absolute(self):
        if self._is_absolute_ is None:
            self._is_absolute_ = MagicMock(return_value=self._path.startswith("/"))
        return self._is_absolute_

    @property
    def exists(self):
        if self._exists_ is None:
            self._exists_ = MagicMock(return_value=self.kwargs.get("exists", True))
    ...
    def glob(self):
        if self._glob_ is None:
            self._glob_ = MagicMock(return_value=self._glob_results)
        return self._glob_
