...
class TestOne(unittest.TestCase):
    ...
    def setUp(self):
        patcher = patch("__main__.isinstance", return_value=True)
        self.isinstance = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("importlib.util.resolve_name", return_value="foo")
        self.resolve_name = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("sys.modules", dict(bar=True))
        self.modules = patcher.start()
        self.addCleanup(patcher.stop)
        spec = types.SimpleNamespace()
        spec.loader = types.SimpleNamespace()
        patcher = patch("importlib.util.find_spec", return_value=spec)
        self.find_spec = patcher.start()
        self.addCleanup(patcher.stop)
        lazy_loader = types.SimpleNamespace()
        lazy_loader.exec_module = MagicMock(return_value=None)
        patcher = patch("flutils.moduleutils._LazyLoader", return_value=lazy_loader)
...
class TestTwo(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.resolve_name = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("sys.modules", dict(foo=True))
        self.modules = patcher.start()
        self.addCleanup(patcher.stop)
        spec = types.SimpleNamespace()
        spec.loader = types.SimpleNamespace()
...
class TestThree(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.resolve_name = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("sys.modules", dict())
        self.modules = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("importlib.util.find_spec", return_value=None)
        self.find_spec = patcher.start()
        self.addCleanup(patcher.stop)
...
class TestFour(unittest.TestCase):
    ...
    def setUp(self):
        ...
        spec = types.SimpleNamespace()
        spec.loader = types.SimpleNamespace()
        spec.loader.create_module = MagicMock(return_value=types.SimpleNamespace())
        patcher = patch("importlib.util.find_spec", return_value=spec)
        self.find_spec = patcher.start()
        self.addCleanup(patcher.stop)
        lazy_loader = types.SimpleNamespace()
