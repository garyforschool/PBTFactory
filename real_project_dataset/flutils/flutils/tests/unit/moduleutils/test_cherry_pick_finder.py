...
class TestOne(unittest.TestCase, CherryPickingMixin):
    ...
    def setUp(self):
        self.meta_path = list()
        patcher = patch("flutils.moduleutils.sys.meta_path", self.meta_path)
        patcher.start()
    ...
    def test_cherry_pick_finder_add(self):
        ...
        _CherryPickFinder.add(
            fullname, "__init__", "apath", self.attr_map, **self.additional_attrs
        )
        ...
        self.assertEqual(self.attr_map, finder._cache[fullname]["attr_map"])
        self.assertEqual(self.additional_attrs, finder._cache[fullname]["addtl_attrs"])
