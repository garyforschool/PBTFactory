...
from flutils.moduleutils import _CHERRY_PICK, _CherryPickingLoader
...
class Test(unittest.TestCase, CherryPickingMixin):
    ...
    def setUp(self):
        patcher = patch(
            "flutils.moduleutils._parse_attr_map", return_value=self.cherry_pick_map
        )
        self.parse_attr_map = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.moduleutils._CherryPickingModule", types.ModuleType)
        patcher.start()
        self.addCleanup(patcher.stop)
