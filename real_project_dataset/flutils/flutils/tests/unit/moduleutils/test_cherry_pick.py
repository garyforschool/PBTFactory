...
class TestOne(unittest.TestCase, CherryPickingMixin):
    ...
    def setUp(self):
        ...
        self.spec = ModuleSpec(
            ...
            sentinel.loader,
            loader_state=dict(
                attr_map=self.attr_map, addtl_attrs=self.additional_attrs
            ),
        )
        patcher = patch(
            "flutils.moduleutils.importlib.util.find_spec", return_value=self.spec
        )
        self.find_spec = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.moduleutils._CherryPickFinder.add", return_value=None)
        self.add = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.moduleutils.sys.modules", dict(testmod=sentinel.testmod)
        )
        self.modules = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.moduleutils.importlib.reload", return_value=None)
...
class TestTwo(unittest.TestCase, CherryPickingMixin):
    ...
    def setUp(self):
        ...
        self.add = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.moduleutils.sys.modules", dict())
        self.modules = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch("flutils.moduleutils.importlib.reload", return_value=None)
        self.reload = patcher.start()
...
class TestThree(unittest.TestCase, CherryPickingMixin):
    ...
    def setUp(self):
        ...
        patcher = patch(
            "flutils.moduleutils.importlib.util.find_spec", return_value=None
        )
        self.find_spec = patcher.start()
        self.addCleanup(patcher.stop)
...
class TestFour(unittest.TestCase, CherryPickingMixin):
    ...
    def setUp(self):
        ...
        self.namespace["__path__"] = ["/home/test_user/tmp/flutils"]
        self.namespace["__attr_map__"] = self.attr_map
        self.namespace["__additional_attrs__"] = {"a": "foo", (22): "bar"}
        self.spec = ModuleSpec(
            "testmod",
            sentinel.loader,
            loader_state=dict(
